      ...
      {open && item && (
        <>
          ...
          <motion.div
            ...
            style={{ backgroundColor: "#151922", border: "1px solid rgba(255,255,255,0.08)", borderBottom: "none" }}
          >
            ...
            <div className="px-5 pt-3 pb-4" style={{ borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              ...
              <div className="flex items-center gap-3">
                <div className="h-14 w-14 shrink-0 overflow-hidden rounded-[16px] bg-[#1A1F2B]">
                  {item.image ? (
                    <img src={item.image} alt={item.name} className="h-full w-full object-cover" />
                  ) : (
                    <div className="flex h-full w-full items-center justify-center">
                      <Camera size={18} className="text-[#3A4255]" />
                    </div>
                  )}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-[15px] font-semibold text-[#F6F3EE]">{item.name}</p>
                  <p className="text-[12px] text-[#6F7788]">{item.category}</p>
                </div>
                <button
                  onClick={onClose}
                  className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/6 text-[#A8AFBE] transition hover:bg-white/12"
                  aria-label="Close"
                >
                  <X size={15} />
                </button>
            ...
            <div className="max-h-[65vh] overflow-y-auto px-5 pb-10 pt-5">
              ...
              <div className="mb-6">
                <p className="mb-1 text-[11px] font-semibold uppercase tracking-widest text-[#5A6275]">
                  ...
                  {REACH_OPTIONS.map((opt) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {active && (
                            <div className="h-2 w-2 rounded-full bg-[#0F1115]" />
                          )}
                        </div>
                        ...
                        <div className="min-w-0 flex-1">
                          <p
                            ...
                            style={{ color: active ? accent : "#F6F3EE" }}
                          >
                            {opt.label}
                          </p>
                          <p className="text-[12px] text-[#6F7788]">{opt.sub}</p>
                        </div>
                      </button>
                    )
                  })}
              ...
              <div>
                ...
                <p className="mb-3 text-[12px] text-[#4A5265]">Optional — helps us style it for you</p>
                  ...
                  {FEEL_OPTIONS.map((opt) => {
                    ...
                    return (
                      <button
                        ...
                        style={{
                          borderColor: active ? "rgba(255,77,141,0.45)" : "rgba(255,255,255,0.08)",
                          backgroundColor: active ? "rgba(255,77,141,0.10)" : "rgba(255,255,255,0.04)",
                          color: active ? "#FF4D8D" : "#A8AFBE",
                        }}
                      >
                ...
                {pref.reach && (
                  ...
                  >
                    <p className="text-[12px] leading-relaxed text-[#6F7788]">
                      {pref.reach === "go-to" &&
