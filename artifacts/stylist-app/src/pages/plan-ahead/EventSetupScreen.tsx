    ...
    <AppShell>
      <header className="mb-6 flex items-center gap-3 pt-4">
        ...
        <div>
          ...
          <p className="text-xs text-[#6B8490]">Tell us about the occasion</p>
        </div>
      </header>

      <section>
        <p className="mb-3 text-sm font-medium text-[#AABBC0]">What's the event?</p>
          ...
          {eventTypes.map(({ type, label, emoji, hint }) => (
            <button
              ...
              className={`flex flex-col items-start gap-1 rounded-[20px] border p-4 text-left transition active:scale-[0.98] ${
                selectedType === type
                  ? "border-[#3F6F73]/60 bg-[#3F6F73]/10"
                  : "border-white/8 bg-[#2A3645] hover:bg-white/5"
              }`}
            >
              <span className="text-2xl">{emoji}</span>
              <span className="text-sm font-semibold text-[#F2F4F5]">{label}</span>
              <span className="text-[11px] leading-4 text-[#6B8490]">{hint}</span>
            </button>
          ))}
        </div>
      </section>
      ...
      <section className="mt-6">
        <p className="mb-3 text-sm font-medium text-[#AABBC0]">Event name (optional)</p>
        <input
          type="text"
          value={customName}
          ...
          className="w-full rounded-[16px] border border-white/10 bg-[#2A3645] px-4 py-3.5 text-sm text-[#F2F4F5] placeholder-[#6B8490] outline-none focus:border-white/25 transition"
        />
      </section>

      <section className="mt-5">
        <p className="mb-3 text-sm font-medium text-[#AABBC0]">When is it?</p>
        <input
          type="date"
          value={date}
          min={todayString()}
          onChange={(e) => setDate(e.target.value)}
          className="w-full rounded-[16px] border border-white/10 bg-[#2A3645] px-4 py-3.5 text-sm text-[#F2F4F5] outline-none focus:border-white/25 transition [color-scheme:dark]"
        />
      </section>

      {date && new Date(date) <= new Date(todayString()) && (
        <p className="mt-2 text-xs text-[#7FA9A3]">Please choose a future date.</p>
      )}

      {isValid && (
        <Card className="mt-5 bg-gradient-to-br from-[#1A2635] to-[#2A3645]">
          <p className="text-xs text-[#AABBC0]">Planning for</p>
          <p className="mt-1 text-[15px] font-semibold text-[#F2F4F5]">{eventName}</p>
