    ...
    return (
      <AppShell>
        <div className="flex h-full flex-col items-center justify-center gap-4 pt-20">
          <p className="text-[#A8AFBE]">Event not found.</p>
      ...
      <header className="mb-5 flex items-center gap-3 pt-4">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            <h1 className="truncate text-[20px] font-semibold tracking-[-0.3px] text-[#F6F3EE]">
              {event.name}
            </h1>
          </div>
          <p className="mt-0.5 text-xs text-[#6F7788]">{formatEventDate(event.date)}</p>
        </div>
      </header>

      <Card
        elevated
        className="bg-gradient-to-br from-[#1E1628] via-[#1A1520] to-[#151922]"
      >
        <div className="flex items-center justify-between">
          <div>
            <p className="text-xs text-[#6F7788]">Time remaining</p>
            <p className="mt-1 text-[32px] font-bold leading-none tracking-[-1px] text-[#F6F3EE]">
              {days > 0 ? days : 0}
              <span className="ml-1 text-sm font-normal text-[#A8AFBE]">days</span>
            </p>
          </div>
          <div className="text-right">
            <p className="text-xs text-[#6F7788]">Delivery window</p>
            <p className="mt-1 text-sm font-semibold" style={{ color: statusColor }}>
              {status}
            </p>
            {status === "Too late" && (
              <p className="mt-0.5 text-[11px] text-[#6F7788]">Express options only</p>
            )}
            {status === "Risky delivery" && (
              <p className="mt-0.5 text-[11px] text-[#6F7788]">Order today to be safe</p>
            )}
            {status === "Safe delivery" && (
              <p className="mt-0.5 text-[11px] text-[#6F7788]">Plenty of time to shop</p>
            )}
          </div>
        </div>

        {isComplete && (
          <div className="mt-4 rounded-[14px] bg-[#4ECFA8]/10 px-4 py-3 text-center">
            <p className="text-sm font-semibold text-[#4ECFA8]">
      ...
      <section className="mt-5">
        ...
        {hasOutfit ? (
          <>
            <div className="grid grid-cols-2 gap-3">
              {outfit.items.slice(0, 4).map((item) => (
                <div
                  key={item.id}
                  className="overflow-hidden rounded-[20px] border border-white/8 bg-[#151922]"
                >
                  ...
                  <div className="p-2">
                    ...
                    <p className="mt-0.5 text-[11px] text-[#6F7788]">{item.category}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            {outfit.items.length > 4 && (
              <p className="mt-2 text-center text-xs text-[#6F7788]">
            ...
            <div className="mt-4 flex gap-3">
              <button
                ...
                className="flex h-11 flex-1 items-center justify-center gap-2 rounded-[14px] border border-white/10 bg-white/5 text-sm font-medium text-[#F6F3EE] transition active:scale-[0.98]"
              >
                <Box size={15} />
                Edit Outfit
              </button>
          ...
          <Card className="text-center py-6">
            ...
            <h3 className="mt-3 text-[16px] font-semibold text-[#F6F3EE]">
              No outfit planned yet
            </h3>
            <p className="mx-auto mt-2 max-w-[240px] text-sm text-[#A8AFBE]">
            ...
            <div className="mt-5 flex flex-col gap-3">
              ...
              <button
                onClick={() => navigate(`/plan-ahead/${eventId}/suggestions`)}
                className="h-11 w-full rounded-[16px] border border-white/10 bg-white/5 text-sm text-[#F6F3EE] transition active:scale-[0.98]"
              >
      ...
      <section className="mt-5">
        <Card className="flex items-center gap-4 opacity-50">
          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[14px] bg-white/8">
            <Lock size={18} className="text-[#A8AFBE]" />
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-[#F6F3EE]">3D Try-On</p>
            <p className="text-xs text-[#6F7788]">Coming later — see your outfit on a virtual model</p>
          </div>
          <span className="shrink-0 rounded-full bg-white/8 px-2.5 py-1 text-[10px] font-medium text-[#A8AFBE]">
            Soon
          </span>
        </Card>
      </section>
