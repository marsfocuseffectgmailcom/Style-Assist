...
const confidenceLabel: Record<string, { label: string; color: string }> = {
  high: { label: "High match", color: "#4ECFA8" },
  safe: { label: "Safe pick", color: "#C8A96A" },
  experimental: { label: "Bold pick", color: "#FF7A5C" },
}
  ...
  return (
    <div className="fixed inset-0 z-[90] flex flex-col justify-end">
      ...
      <motion.div
        ...
        className="relative rounded-t-[28px] bg-[#151922] px-5 pb-10 pt-4"
      >
        <div className="mb-3 flex items-center justify-between">
            ...
            {STEP_ORDER_DEFAULT.map((_, i) => (
              <div
                ...
                className={`h-1 rounded-full transition-all duration-300 ${
                  i < stepNumber ? "bg-[#FF4D8D] w-5" : i === stepNumber - 1 ? "bg-[#FF4D8D] w-5" : "bg-white/15 w-3"
                }`}
              />
            ))}
          </div>
          <button
            onClick={finishFTE}
            className="flex h-7 w-7 items-center justify-center rounded-full bg-white/8 text-[#6F7788]"
          >
            <X size={14} />
          </button>
        </div>
...
function TodayOutfitStep({
  ...
  return (
    <div>
      <p className="mb-1 text-[11px] font-semibold uppercase tracking-[0.1em] text-[#FF4D8D]">
        Your outfit for today
      </p>
      <h3 className="mb-4 text-[22px] font-bold tracking-[-0.4px] text-[#F6F3EE]">
        {outfit ? "Looking good." : "Let's start."}
      </h3>

      {outfit ? (
        <div className="mb-5 flex items-center gap-3 rounded-[20px] border border-white/10 bg-[#1A1F2B] p-3">
          <div className="h-[72px] w-[72px] shrink-0 overflow-hidden rounded-[14px] bg-[#111519]">
            <OutfitMini items={outfit.items} />
          </div>
          <div>
            <p className="text-[15px] font-semibold text-[#F6F3EE]">{outfit.name}</p>
            <p className="mt-0.5 text-[12px] text-[#6F7788]">
        ...
        <div className="mb-5 rounded-[20px] border border-dashed border-white/15 bg-white/3 py-6 text-center">
          <p className="text-sm text-[#6F7788]">Generating your first outfit…</p>
        </div>
      )}

      <button
        onClick={onKeep}
        className="mb-2.5 flex w-full items-center justify-center gap-2 rounded-[18px] bg-gradient-to-r from-[#FF4D8D] to-[#FF7A5C] py-3.5 text-[15px] font-semibold text-white transition active:scale-[0.97]"
      >
        <Check size={16} />
        Looks great, keep it
      </button>
      <button
        onClick={onSwap}
        className="w-full rounded-[18px] border border-white/10 bg-white/5 py-3.5 text-[14px] text-[#A8AFBE] transition active:scale-[0.97]"
      >
  ...
  return (
    <div>
            ...
            <button
              ...
              className={`flex w-full items-center gap-3 rounded-[18px] border p-3 text-left transition active:scale-[0.98] ${
                isSelected
                  ...
                  : "border-white/8 bg-[#1A1F2B]"
              }`}
            >
              <div className="h-14 w-14 shrink-0 overflow-hidden rounded-[12px] bg-[#111519]">
                <OutfitMini items={alt.items} />
              </div>
              <div className="min-w-0 flex-1">
                <p className="truncate text-[13px] font-semibold text-[#F6F3EE]">{alt.name}</p>
                <span
      ...
      <button
        onClick={onApply}
        disabled={!selected}
        className="w-full rounded-[18px] bg-gradient-to-r from-[#FF4D8D] to-[#FF7A5C] py-3.5 text-[15px] font-semibold text-white transition active:scale-[0.97] disabled:opacity-50"
      >
        Use this look
      </button>
    </div>
...
function WardrobeAddStep({
  ...
  return (
    <div>
      ...
      <h3 className="mb-1 text-[22px] font-bold tracking-[-0.4px] text-[#F6F3EE]">
        Tap items you own.
      </h3>
      <p className="mb-4 text-[13px] text-[#6F7788]">
        Select at least 3 for better outfit matches.{" "}
        <span style={{ color: enough ? "#4ECFA8" : "#6F7788" }}>
            ...
            >
              <div
                className={`relative h-[72px] w-full overflow-hidden rounded-[14px] border-2 transition ${
                  active ? "border-[#FF4D8D]" : "border-transparent"
                }`}
              >
                <img src={item.image} alt={item.name} className="h-full w-full object-cover" />
                {active && (
                  <div className="absolute inset-0 flex items-center justify-center bg-[#FF4D8D]/30">
                    <Check size={18} className="text-white drop-shadow" />
                  </div>
                )}
              </div>
              <span className="text-center text-[10px] text-[#A8AFBE]">{item.name}</span>
            </motion.button>
          )
        })}
      </div>
      ...
      <AnimatePresence>
        {enough && (
          <motion.button
            ...
            animate={{ opacity: 1, y: 0 }}
            onClick={onNext}
            className="flex w-full items-center justify-center gap-2 rounded-[18px] bg-gradient-to-r from-[#FF4D8D] to-[#FF7A5C] py-3.5 text-[15px] font-semibold text-white transition active:scale-[0.97]"
          >
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex items-center gap-3 rounded-[18px] border border-[#C8A96A]/25 bg-[#C8A96A]/6 p-3.5">
        <div className="h-14 w-14 shrink-0 overflow-hidden rounded-[12px] bg-[#1A1F2B]">
          <img
        ...
        <div>
          <p className="text-[14px] font-semibold text-[#F6F3EE]">White Linen Shirt</p>
          <p className="text-[12px] text-[#6F7788]">ASOS · Ready to add</p>
        </div>
        <Package size={16} className="ml-auto shrink-0 text-[#C8A96A]" />
      </div>

      <p className="mb-2 text-[12px] font-medium text-[#A8AFBE]">When does it arrive?</p>
        ...
        {([3, 7, 14] as const).map((d) => (
          <button
            ...
            className={`flex-1 rounded-[14px] border py-2.5 text-[13px] font-semibold transition active:scale-[0.97] ${
              days === d
                ? "border-[#FF4D8D]/40 bg-[#FF4D8D]/15 text-[#FF4D8D]"
      ...
      <button
        onClick={onAdd}
        className="w-full rounded-[18px] bg-gradient-to-r from-[#FF4D8D] to-[#FF7A5C] py-3.5 text-[15px] font-semibold text-white transition active:scale-[0.97]"
      >
        Add delivery
      </button>
    </div>
...
function DeliveryUnlocksStep({
  ...
  return (
    <div>
      ...
      <p className="mb-5 text-[13px] text-[#6F7788]">
      ...
      <div className="mb-5 flex items-center gap-3">
        <div className="h-[52px] w-[52px] shrink-0 overflow-hidden rounded-[12px] bg-[#1A1F2B]">
          <img
            ...
            <AnimatePresence key={i}>
              {i < visible && (
                <motion.div
                  ...
                  className="h-[52px] w-[52px] overflow-hidden rounded-[12px] bg-[#1A1F2B]"
                >
                  <div className="grid h-full w-full grid-cols-2 gap-0.5">
                    {o.images.map((src, j) => (
      ...
      <motion.div
        ...
        className="mb-5 rounded-[14px] bg-[#4ECFA8]/10 px-4 py-2.5 text-[13px] font-medium text-[#4ECFA8]"
      >
...
function PlanMonthStep({
  ...
  return (
    <div>
      ...
      <p className="mb-6 text-[13px] text-[#6F7788]">
        {done
      ...
      {done ? (
        <motion.div
          ...
          className="mb-5 flex items-center gap-3 rounded-[18px] bg-[#4ECFA8]/10 p-4"
        >
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#4ECFA8]/20">
            <Check size={20} className="text-[#4ECFA8]" />
          </div>
          <div>
            <p className="text-[14px] font-semibold text-[#4ECFA8]">28 days planned</p>
            <p className="text-[12px] text-[#6F7788]">Ready to view your timeline</p>
          </div>
        </motion.div>
      ) : (
        <div className="mb-5 grid grid-cols-4 gap-1.5">
          {Array.from({ length: 16 }).map((_, i) => (
            <div
              key={i}
              className="h-9 rounded-[10px] border border-white/8 bg-[#1A1F2B]"
            />
          ))}
        </div>
      )}
      ...
      <button
        ...
        className="flex w-full items-center justify-center gap-2 rounded-[18px] bg-gradient-to-r from-[#FF4D8D] to-[#FF7A5C] py-3.5 text-[15px] font-semibold text-white shadow-[0_4px_20px_rgba(255,77,141,0.28)] transition active:scale-[0.97] disabled:opacity-60"
      >
        {planning ? (
          <>
            <motion.div
...
function AllSetStep({ onAllow, onSkip }: { onAllow: () => void; onSkip: () => void }) {
  ...
  return (
    <div>
      <div className="mb-4 flex h-14 w-14 items-center justify-center rounded-full bg-[#4ECFA8]/15">
        <Check size={26} className="text-[#4ECFA8]" />
      </div>
      <h3 className="mb-1 text-[22px] font-bold tracking-[-0.4px] text-[#F6F3EE]">
        You&apos;re all set.
      </h3>
      ...
      {!asked ? (
        <>
          <div className="mb-4 flex items-start gap-3 rounded-[16px] border border-white/8 bg-[#1A1F2B] p-4">
            <CalendarDays size={18} className="mt-0.5 shrink-0 text-[#FF4D8D]" />
            <div>
              <p className="text-[13px] font-semibold text-[#F6F3EE]">Get reminders</p>
              <p className="mt-0.5 text-[12px] text-[#6F7788]">
          ...
          <button
            onClick={onSkip}
            className="w-full rounded-[18px] border border-white/10 bg-white/5 py-3.5 text-[14px] text-[#6F7788] transition active:scale-[0.97]"
          >
            Skip for now
          </button>
        </>
        ...
        <motion.div
          ...
          animate={{ opacity: 1, y: 0 }}
          className="rounded-[16px] bg-[#4ECFA8]/10 p-4 text-center text-[13px] text-[#4ECFA8]"
        >
